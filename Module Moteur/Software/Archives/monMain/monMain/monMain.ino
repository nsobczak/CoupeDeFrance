    ...
    */
    ...
    if (StopIR == 0)
    {
        ...
        if (millis() - timer >= 100)
        {
            ...
            Serial.println(destination.thetha);
            Serial.print("\t roueCodeuse.thetha : \t");
            ...
            if (((X <= DX + 50) && (X >= DX - 50)) || (memory_destination.x == destination.x))
            {
                ...
                if (((Y <= DY + 50) && (Y >= DY - 50)) || (memory_destination.y == destination.y))
                {
                    ...
                    if ((ANGLE <= DA + 10) && (ANGLE >= DA - 10))
                    {
                        ...
                        */
                        Serial.println("___entering straightForward___");
                        straightForward(destination, roueCodeuse, robot, roue, codeuse);
                        Serial.println("exiting straightForward___");
                        /*
                        Serial.println("___entering keepPosition___");
                        keepPosition(destination, roueCodeuse, robot, roue, codeuse);
                        Serial.println("exiting keepPosition___");
                        */
...
/**
 * \fn  void straightForward(Position destination, Position roueCodeuse, Asservissement robot, Odometrie roue, Tick codeuse)
 * \brief fonction qui fait aller droit le robot
 */
void straightForward(Position destination, Position roueCodeuse, Asservissement robot, Odometrie roue, Tick codeuse)
{
...
void errorLT()
{
    ...
    if (roueCodeuse.distance != 0)
    {
        ...
        if (destination.x < roueCodeuse.x)
        {//if target is negative
            if (beta >= 0)
            {//target on the left (in the work plan)
                beta = PI - beta;
            } else
            {//right
                beta = -PI - beta;
    ...
    {//else target is on robot position
        beta = 0;
    }
    ...
    //errorT=depassement;
    if (robot.abs1(depassement) <= 90)
    {//if target ahead robot
        forward = -1; //forward
    } else
